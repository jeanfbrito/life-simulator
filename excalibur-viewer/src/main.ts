...
/**
 * Main entry point for the Life Simulator Excalibur Viewer
 */
async function main(): Promise<void> {
  const logger = new Logger('Main')

  try {
    logger.info('Starting Life Simulator Excalibur Viewer...')

    // Check URL parameters for debug mode
    const urlParams = new URLSearchParams(window.location.search)
    const enableDebug = urlParams.has('debug') || urlParams.has('d')
    const logLevel = (urlParams.get('log') as any) || (enableDebug ? 'debug' : 'info')
    const enableHiDPI = !urlParams.has('no-hidpi')

    // Initialize and start the engine with configuration
    ...
    logger.info('Life Simulator started successfully')
    logger.info(`Configuration: Debug=${enableDebug}, HiDPI=${enableHiDPI}, Display Scale=${engine.getDisplayScale()}`)
    ...
    logger.error('Failed to start Life Simulator:', error)
