...
fn test_herbivore_consumption_integration() {
    ...
    println!(
        "Initial vegetation biomass at (5,5): {}",
        initial_vegetation
    );
    ...
    let initial_hunger = {
        ...
        hunger_query.iter(app.world()).next().unwrap().0 .0.current
    };
    ...
    let final_hunger = {
        let mut hunger_query = app.world_mut().query::<&Hunger>();
    ...
    assert!(
        ...
        "Rabbit hunger should have been reduced"
    );
    ...
    println!(
        "Expected consumption: {}, Actual consumption: {}, Hunger reduction: {}",
        expected_consumption, actual_consumption, hunger_reduction
    );

    // Allow for small floating point differences
    assert!(
        (actual_consumption - expected_consumption).abs() < 0.1,
        "Consumed amount should match 30% rule"
    );
    assert!(
        (hunger_reduction - actual_consumption).abs() < 0.1,
        "Hunger reduction should match consumed amount"
    );
}

fn setup_test_world(mut resource_grid: ResMut<ResourceGrid>) {
    // Create a simple test world with grass at position (5,5)
    let test_tile = IVec2::new(5, 5);

    // Add vegetation cell to the test tile with high biomass
    let cell = resource_grid.get_or_create_cell(test_tile, 100.0, 1.0); // Max biomass, normal growth rate
    cell.total_biomass = 80.0; // High biomass for testing
