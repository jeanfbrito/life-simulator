use bevy::math::IVec2;
use life_simulator::pathfinding::{PathfindingGrid, find_path};
...
fn test_rabbit_can_path_to_water() {
    ...
    let world_loader = WorldLoader::load_default()
        .expect("Failed to load world - run map_generator first");
    ...
    println!("✅ Grid built: {} tiles processed, {} walkable, {} blocked by resources", 
    ...
    for (name, spawn_pos) in test_cases {
        ...
        match water_pos {
            Some((water_x, water_y)) => {
                let distance = ((water_x - spawn_pos.x).pow(2) + (water_y - spawn_pos.y).pow(2)) as f32;
                let distance = distance.sqrt();
                println!("   💧 Found water at ({}, {}) - distance: {:.1} tiles", water_x, water_y, distance);
                
                // Find adjacent walkable tile
                let adjacent = find_adjacent_walkable_to_water(IVec2::new(water_x, water_y), &world_loader);
                ...
                match adjacent {
                    Some(adj_pos) => {
                        ...
                        // Try pathfinding
                        println!("   🧭 Finding path from {:?} to {:?}...", spawn_pos, adj_pos);
                        let path = find_path(
                            spawn_pos,
                            adj_pos,
                            &pathfinding_grid,
                        ...
                        match path {
                            ...
                            None => {
                                ...
                                for i in 0..=steps.min(20) {  // Sample up to 20 points
                                    let t = if steps > 0 { i as f32 / steps as f32 } else { 0.0 };
                                    let sample_x = (spawn_pos.x as f32 + dx as f32 * t).round() as i32;
                                    let sample_y = (spawn_pos.y as f32 + dy as f32 * t).round() as i32;
                                    let sample_pos = IVec2::new(sample_x, sample_y);
                                    
                                    if let Some(terrain_str) = world_loader.get_terrain_at(sample_x, sample_y) {
                                        let walkable = pathfinding_grid.is_walkable(sample_pos);
                                        if !walkable {
                                            impassable_count += 1;
                                            let resource_str = world_loader.get_resource_at(sample_x, sample_y)
                                ...
                                println!("      Impassable tiles found: {}/{}", impassable_count, steps.min(20) + 1);
                            }
                        }
                    }
                    None => {
...
fn find_nearest_shallow_water(from: IVec2, world_loader: &WorldLoader, max_radius: i32) -> Option<(i32, i32)> {
