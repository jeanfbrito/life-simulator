...
use life_simulator::simulation::{accumulate_ticks, should_tick, SimulationSpeed, SimulationTick};
...
fn test_entity_movement_timing() {
    ...
    app.add_systems(
        Update,
        (wanderer_ai_system, movement_tick_system).run_if(should_tick),
    );
...
fn test_long_running_stability() {
    ...
    println!(
        "After 30 seconds: {} ticks, should_tick={}",
        ticks, should_tick
    );
}
...
fn test_systems_respect_should_tick_condition() {
    ...
    let entity = app.world.spawn_empty().id();
    app.world.entity_mut(entity).insert({
        use life_simulator::entities::{EntityName, MovementSpeed, TilePosition};
        (
            EntityName("TestHuman".to_string()),
            TilePosition::from_xy(50, 50),
            MovementSpeed { ticks_per_tile: 30 },
