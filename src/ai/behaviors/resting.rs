...
use bevy::prelude::*;
use crate::entities::{TilePosition, stats::Energy};
...
mod tests {
    ...
    fn test_rest_when_low_energy() {
        ...
        assert!(result.is_some(), "Should want to rest at 20% energy with 30% threshold");
        
        let score = result.unwrap();
        assert!(matches!(score.action_type, ActionType::Rest { .. }));
        assert!(score.utility > 0.7, "Utility should be high when very tired");
    }
    ...
    fn test_no_rest_when_high_energy() {
        ...
        assert!(result.is_none(), "Should NOT rest at 80% energy with 30% threshold");
    }
